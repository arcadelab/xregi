...
def transunet_feature_extractor(model, x):
    if x.size()[1] == 1:
        x = x.repeat(1, 3, 1, 1)
    x, attn_weights, features = model.transformer(x)  # (B, n_patch, hidden)
    x = model.decoder(x, features)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False,
                 num_lands=0, lands_block_depth=0, lands_num_1x1=2,
                 n_classes=2, batch_norm=False, padding=False, pad_mode='zeros',
                 do_soft_max=True):
        ...
        # Concurrent landmark detection
        if self.num_lands > 0:
            lands_use_non_lin = False
            self.lands_block = None
            prev_channels = config['decoder_channels'][-1]
            ...
            if lands_block_depth > 0:
                ...
                if lands_use_non_lin:
                    ...
                    if batch_norm:
                        lands_block.append(
                            nn.BatchNorm2d(lands_block_num_chan))
...
class VisionTransformerAdv(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False,
                 num_lands=0, lands_block_depth=0, lands_num_1x1=2,
                 n_classes=2, batch_norm=False, padding=False, pad_mode='zeros',
                 do_soft_max=True):
        super(VisionTransformerAdv, self).__init__()
        self.num_lands = num_lands
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        ...
        self.config = config
        self.conv_seg_adv = nn.Conv2d(16, n_classes, kernel_size=1, bias=True)
        self.conv_landmarks_adv = nn.Conv2d(
            16, num_lands, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        seg_adv = self.conv_seg_adv(x_rev)
        if self.num_lands > 0:
            heat_maps_adv = self.conv_landmarks_adv(x_rev)
        ...
        if self.num_lands > 0:
            ...
            return (seg, heat_maps), (seg_adv, heat_maps_adv)
        else:
            return seg, seg_adv
